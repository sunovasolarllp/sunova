...
function handleFormSubmit(event) {
    ...
    if (!district || !location || !dealerCode) {
        showFormFeedback('District, Location, and Partner Area fields are mandatory.', 'error');
        return;
    }
    ...
    if (!phoneRegex.test(phone)) {
        showFormFeedback('Please enter a valid 10-digit mobile number starting with 6, 7, 8, or 9.', 'error');
        return;
    }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup drag and drop for KSEB bill dropzone
    const dropzone = document.getElementById('kseb-dropzone');
    if (dropzone) {
        ...
        dropzone.addEventListener('drop', (e) => {
            ...
            if (files && files.length > 0) {
                const fileInput = document.getElementById('kseb-bill-file');
                if (fileInput) {
                    fileInput.files = files;
                    handleKSEBBillUpload({ target: { files: files } });
                }
            }
        }, false);
    }
...
// =========================================================================
// KSEB Electricity Bill Upload & Monthly Consumption Calculation Feature
// =========================================================================
async function handleKSEBBillUpload(event) {
    const file = event.target.files[0];
    if (!file) return;

    const statusEl = document.getElementById('kseb-upload-status');
    const statusTextEl = document.getElementById('kseb-status-text');
    const resultEl = document.getElementById('kseb-bill-result');

    if (statusEl) statusEl.classList.remove('hidden');
    if (resultEl) resultEl.classList.add('hidden');
    if (statusTextEl) statusTextEl.textContent = `Reading ${file.name}...`;

    try {
        let extractedText = "";

        if (file.type === "application/pdf" || file.name.toLowerCase().endsWith(".pdf")) {
            if (statusTextEl) statusTextEl.textContent = "Extracting text from KSEB PDF bill...";
            extractedText = await parseKSEBPdf(file);
        } else if (file.type.startsWith("image/") || /\.(png|jpe?g|webp)$/i.test(file.name)) {
            if (statusTextEl) statusTextEl.textContent = "Scanning KSEB bill image text (OCR)...";
            extractedText = await parseKSEBImage(file);
        } else {
            throw new Error("Unsupported file format. Please upload a KSEB PDF bill or image (JPG/PNG).");
        }
        ...
        // Parse key numbers (Bill Amount, Units, Consumer No, Name, Period)
        const parsed = parseKSEBBillText(extractedText);

        const totalBill = parsed.billAmount || 0;
        const totalUnits = parsed.totalUnits || 0;

        // Calculate Monthly Values (KSEB Domestic bills in Kerala are typically bi-monthly = 2 months)
        const isBiMonthly = parsed.isBiMonthly !== false; 
        const monthDivisor = isBiMonthly ? 2 : 1;

        currentKSEBBillData.totalBill = totalBill;
        currentKSEBBillData.totalUnits = totalUnits;
        currentKSEBBillData.isBiMonthly = isBiMonthly;

        const monthlyBill = totalBill > 0 ? Math.round(totalBill / monthDivisor) : 0;
        const monthlyUnits = totalUnits > 0 ? Math.round(totalUnits / monthDivisor) : 0;

        // Update Result Summary UI
        const amtInput = document.getElementById('res-bill-amt-input');
        if (amtInput) {
            amtInput.value = totalBill > 0 ? totalBill : '';
            amtInput.placeholder = "Enter Billed Amount (₹)";
        }
        
        const unitsInput = document.getElementById('res-total-units-input');
        if (unitsInput) {
            unitsInput.value = totalUnits > 0 ? totalUnits : '';
            unitsInput.placeholder = "Enter Total Units (kWh)";
        }
        
        const periodEl = document.getElementById('res-bill-period');
        if (periodEl) periodEl.textContent = isBiMonthly ? "Bi-Monthly (2 Months)" : "Monthly (1 Month)";
        
        const mBillEl = document.getElementById('res-monthly-bill');
        if (mBillEl) mBillEl.textContent = monthlyBill > 0 ? `₹${monthlyBill.toLocaleString('en-IN')} / mo` : `₹-- / mo`;
        
        const mUnitsEl = document.getElementById('res-monthly-units');
        if (mUnitsEl) mUnitsEl.textContent = monthlyUnits > 0 ? `${monthlyUnits} kWh / mo` : `-- kWh / mo`;

        const consInfoEl = document.getElementById('res-consumer-info');
        if (parsed.consumerName || parsed.consumerNo || parsed.sectionName) {
            if (consInfoEl) consInfoEl.classList.remove('hidden');
            
            const cNameInput = document.getElementById('res-consumer-name-input');
            if (cNameInput) cNameInput.value = parsed.consumerName || '';
            
            const cNoInput = document.getElementById('res-consumer-no-input');
            if (cNoInput) cNoInput.value = parsed.consumerNo || '';

            const cSecEl = document.getElementById('res-kseb-section');
            if (cSecEl) {
                if (parsed.sectionName && parsed.districtName) {
                    cSecEl.textContent = `${parsed.sectionName} (${parsed.districtName})`;
                } else if (parsed.sectionName) {
                    cSecEl.textContent = parsed.sectionName;
                } else {
                    cSecEl.textContent = '--';
                }
            }

            // Auto-fill consumer number and customer name in Feasibility form if empty
            const consFormInput = document.getElementById('form-consumer-no');
            if (consFormInput && parsed.consumerNo && !consFormInput.value) {
                consFormInput.value = parsed.consumerNo;
            }
            const nameFormInput = document.getElementById('form-name');
            if (nameFormInput && parsed.consumerName && !nameFormInput.value) {
                nameFormInput.value = parsed.consumerName.toUpperCase();
            }

            // Auto-select District & KSEB Section dropdowns
            if (parsed.districtName) {
                const distSelect = document.getElementById('form-district');
                if (distSelect) {
                    distSelect.value = parsed.districtName;
                    if (typeof handleDistrictChange === 'function') {
                        handleDistrictChange(parsed.districtName);
                    }
                }
            }
            ...
            if (parsed.sectionName) {
                const secSelect = document.getElementById('form-location');
                if (secSelect) {
                    const opts = Array.from(secSelect.options);
                    const matchedOpt = opts.find(o => o.text.toLowerCase().trim() === parsed.sectionName.toLowerCase().trim() || o.text.toLowerCase().includes(parsed.sectionName.toLowerCase()));
                    if (matchedOpt) {
                        secSelect.value = matchedOpt.value;
                        if (typeof handleKSEBSectionChange === 'function') {
                            handleKSEBSectionChange(matchedOpt.value);
                        }
                    }
                }
            }
        ...
        // Automatically update the solar calculator inputs & recalculate!
        if (typeof handleBillManualInput === 'function') {
            handleBillManualInput(monthlyBill);
        }
        if (typeof handleUnitInput === 'function') {
            handleUnitInput(monthlyUnits);
        }

        // Scroll smoothly to output
        const calcOutputs = document.querySelector('.calc-results-panel');
        if (calcOutputs) {
            calcOutputs.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        }

    } catch (err) {
        console.error("KSEB Bill Parsing Error:", err);
        if (statusEl) statusEl.classList.add('hidden');
        alert(`KSEB Bill Reader: ${err.message || err}`);
    }
}

let currentKSEBBillData = {
    totalBill: 0,
    totalUnits: 0,
    isBiMonthly: true
};

function handleExtractedNameEdit(val) {
    const nameFormInput = document.getElementById('form-name');
    if (nameFormInput) {
        nameFormInput.value = val.toUpperCase();
    }
}

function handleExtractedConsNoEdit(val) {
    const consFormInput = document.getElementById('form-consumer-no');
    if (consFormInput) {
        consFormInput.value = val.replace(/[^0-9]/g, '');
    }
}

function handleExtractedAmountEdit(val) {
    const amt = parseFloat(val);
    if (!isNaN(amt) && amt > 0) {
        currentKSEBBillData.totalBill = Math.round(amt);
        const factor = currentKSEBBillData.isBiMonthly ? 2 : 1;
        const monthlyBill = Math.round(currentKSEBBillData.totalBill / factor);
        
        const mBillEl = document.getElementById('res-monthly-bill');
        if (mBillEl) mBillEl.textContent = `₹${monthlyBill.toLocaleString('en-IN')} / mo`;
...
function handleExtractedUnitsEdit(val) {
    const units = parseInt(val, 10);
    if (!isNaN(units) && units > 0) {
        currentKSEBBillData.totalUnits = units;
        const factor = currentKSEBBillData.isBiMonthly ? 2 : 1;
        const monthlyUnits = Math.round(currentKSEBBillData.totalUnits / factor);
        
        const mUnitsEl = document.getElementById('res-monthly-units');
        if (mUnitsEl) mUnitsEl.textContent = `${monthlyUnits} kWh / mo`;
...
// Image text extractor via Tesseract.js
async function parseKSEBImage(file) {
    if (typeof Tesseract === 'undefined') {
        throw new Error("OCR library is initializing. Please wait a moment and try again.");
    }
    const worker = await Tesseract.createWorker('eng');
    const ret = await worker.recognize(file);
    await worker.terminate();
...
// KSEB Bill Text Parser
function parseKSEBBillText(text) {
    console.log("--- KSEB BILL PARSER DEBUG START ---");
    console.log(text);
    ...
    let isBiMonthly = true;

    // Normalize text line by line
    const lines = text.split(/\r?\n/).map(l => l.trim()).filter(Boolean);

    // Set of numbers associated with consumer number or meter readings to strictly exclude from being parsed as bill amount
    const numbersToExclude = new Set();

    // =========================================================================
    // 1. EXTRACT CONSUMER NUMBER (13 digits starting with 1, e.g. 1155043005373)
    // =========================================================================
    let consNoLineIndex = -1;
    for (let i = 0; i < lines.length; i++) {
        const m = lines[i].match(/(?:Consumer\s*No|Consumer\s*Number|Cons\.?\s*No\.?|Cons\s*No|Account\s*No)[\s\:\-\=]*([0-9]{13})/i) || lines[i].match(/\b([1-9][0-9]{12})\b/);
        if (m && m[1]) {
            consumerNo = m[1];
            consNoLineIndex = i;
            const cnInt = parseInt(consumerNo, 10);
            if (!isNaN(cnInt)) numbersToExclude.add(cnInt);
            numbersToExclude.add(parseInt(consumerNo.substring(0, 6), 10));
            numbersToExclude.add(parseInt(consumerNo.substring(6, 13), 10));
            break;
        }
    }
    console.log("Consumer Number to Exclude:", consumerNo);

    // =========================================================================
    // 2. EXTRACT CONSUMER NAME
    // =========================================================================
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        const m = line.match(/(?:Consumer\s*Name|Cons\.?\s*Name|Customer\s*Name|Name\s*&\s*Address|Name)[\s\:\-\=]+([A-Za-z\s\.]{3,35})/i);
        if (m && m[1]) {
            const candidate = m[1].trim();
            if (!/Address|KSEB|Section|Bill|Details|Net|Payable|Total|Electrical|Office|Circle|Division/i.test(candidate) && candidate.length >= 3) {
                consumerName = candidate;
                break;
            }
        }
    }
    if (!consumerName && consNoLineIndex >= 0) {
        const checkIndices = [consNoLineIndex + 1, consNoLineIndex - 1, consNoLineIndex + 2];
        for (const idx of checkIndices) {
            if (idx >= 0 && idx < lines.length) {
                const l = lines[idx];
                if (!/[0-9]{5,}/.test(l) && !/KSEB|Section|Bill|Date|Reading|Units|Amount|Payable/i.test(l)) {
                    const cleanName = l.replace(/[^A-Za-z\s\.]/g, '').trim();
                    if (cleanName.length >= 3 && cleanName.length <= 35) {
    ...
    // Identify meter reading numbers to EXCLUDE them from being mistaken as bill amount!
    for (const line of lines) {
        if (/Prev|Pres|Reading|Rdg|Meter|Serial|Const|MF/i.test(line)) {
            const nums = line.match(/\b[0-9]{3,6}\b/g);
            if (nums) {
                nums.forEach(n => numbersToExclude.add(parseInt(n, 10)));
            }
        }
    }
    console.log("Numbers to Exclude from Bill Amount:", Array.from(numbersToExclude));

    // =========================================================================
    // 3. EXTRACT UNITS CONSUMED (kWh)
    // =========================================================================
    const unitPatterns = [
        /(?:Total\s*Consumed\s*Units|Billed\s*Units|Units\s*Consumed|Cons\.?\s*\(?Units\)?|Cons\.?\s*Units|Total\s*Units|Consumption|Units)[\s\:\-\=\|]*([0-9]{1,5})/i,
        /([0-9]{2,4})\s*(?:Units|kWh)/i
    ];

    for (const line of lines) {
        if (/Prev\s*Rdg|Pres\s*Rdg|Previous\s*Reading|Present\s*Reading|Meter\s*No|Read\s*Date|Consumer\s*No/i.test(line)) continue;

        for (const pattern of unitPatterns) {
            const m = line.match(pattern);
            if (m && m[1]) {
                const u = parseInt(m[1], 10);
                if (u >= 10 && u <= 10000 && !numbersToExclude.has(u)) {
                    totalUnits = u;
                    break;
                }
            }
    ...
    // =========================================================================
    // 4. EXTRACT FINAL NET PAYABLE AMOUNT
    // =========================================================================
    // Direct Pass 1: Target the phrase "Net Payable" or "Net Amount Payable" from bottom to top
    for (let i = lines.length - 1; i >= 0; i--) {
        const line = lines[i];
        if (/Net\s*(?:Amount|Amt)?\s*Payable/i.test(line)) {
            console.log("Direct Pass 1 matched Net Payable line:", line);
            for (let offset = 0; offset <= 4; offset++) {
                if (i + offset < lines.length) {
                    const checkLine = lines[i + offset];
                    const matches = checkLine.match(/\b[0-9]{3,5}(?:\.[0-9]{1,2})?\b/g);
                    if (matches) {
                        for (let mIdx = matches.length - 1; mIdx >= 0; mIdx--) {
                            const val = parseFloat(matches[mIdx]);
                            const valInt = Math.round(val);
                            if (val >= 100 && val <= 75000 && !numbersToExclude.has(valInt)) {
                                billAmount = valInt;
                                console.log("Extracted billAmount via direct Net Payable match:", billAmount);
                                break;
                            }
                        }
                    }
    ...
    if (!billAmount) {
        const netPayableHeaders = [
            /Net\s*(?:Amount|Amt)?\s*Payable/i,
            /Net\s*Payable/i,
            /Total\s*Amount\s*Payable/i,
            /Total\s*Payable/i,
            /Amount\s*Payable/i,
            /Grand\s*Total/i,
            /Total\s*Bill\s*Amount/i,
            /അടയ്‌ക്കേണ്ട\s*തുക/i,
            /നെറ്റ്\s*തുക/i
        ];

        // Search lines from BOTTOM to TOP
        for (let i = lines.length - 1; i >= 0; i--) {
            const line = lines[i];

            // STRICTLY IGNORE any Sub-Total or Breakdown lines!
            if (/Sub\s*Total|Subtotal|Sub-Total|Energy\s*Charge|Fixed\s*Charge|Fuel\s*Surcharge|Duty|Previous\s*Reading|Present\s*Reading/i.test(line) && !/Net\s*Payable|Grand\s*Total/i.test(line)) {
                continue;
            }

            // Check if line contains any Net Payable header
            let isNetPayableLine = false;
            for (const reg of netPayableHeaders) {
                if (reg.test(line)) {
                    isNetPayableLine = true;
                    break;
                }
            }

            if (isNetPayableLine) {
                console.log("Found Net Payable Header (Bottom-Up) on line", i, ":", line);

                // Check line i and lines immediately following it (offsets 0 to 4)
                for (let offset = 0; offset <= 4; offset++) {
                    if (i + offset < lines.length) {
                        const checkLine = lines[i + offset];
                        if (/Sub\s*Total|Subtotal|Energy\s*Charge|Fixed\s*Charge/i.test(checkLine) && offset > 0) continue;

                        const matches = checkLine.match(/\b[0-9]{3,5}(?:\.[0-9]{1,2})?\b/g);
                        if (matches) {
                            // Take the LAST numeric match on the Net Payable line (as final total is at rightmost column)
                            for (let mIdx = matches.length - 1; mIdx >= 0; mIdx--) {
                                const val = parseFloat(matches[mIdx]);
                                const valInt = Math.round(val);
                                if (val >= 100 && val <= 75000 && !numbersToExclude.has(valInt)) {
                                    billAmount = valInt;
                                    console.log("Extracted Final Net Payable Amount:", billAmount, "from line:", checkLine);
                                    break;
                                }
                            }
                        }
    ...
    // Proximity Substring Search Fallback
    if (!billAmount) {
        const netIdx = text.search(/(?:Net\s*(?:Amount|Amt)?\s*Payable|Net\s*Payable|Net\s*Amt|അടയ്‌ക്കേണ്ട\s*തുക)/i);
        if (netIdx !== -1) {
            const snippet = text.substring(netIdx, netIdx + 250);
            const matches = snippet.match(/\b[0-9]{3,5}(?:\.[0-9]{1,2})?\b/g);
            if (matches) {
                for (let mIdx = matches.length - 1; mIdx >= 0; mIdx--) {
                    const val = parseFloat(matches[mIdx]);
                    const valInt = Math.round(val);
                    if (val >= 100 && val <= 75000 && !numbersToExclude.has(valInt)) {
                        billAmount = valInt;
                        console.log("Extracted billAmount via proximity snippet:", billAmount);
                        break;
                    }
                }
            }
    ...
    // Do NOT guess or estimate billAmount if Net Payable was not found cleanly

    // =========================================================================
    // 5. EXTRACT KSEB SECTION & DISTRICT
    // =========================================================================
    if (typeof KSEB_SECTIONS !== 'undefined') {
        const textUpper = text.toUpperCase();

        // Strategy A: Scan for explicit section header line ("Electrical Section : Punnapra", "KSEB Section Punnapra", etc.)
        for (const line of lines) {
            if (/Electrical\s*Section|Section\s*Office|Sec\.?\s*Office|Section/i.test(line) && !/Regional|Circle|Division|Discom|Board/i.test(line)) {
                const lineUpper = line.toUpperCase();
                for (const [dist, sections] of Object.entries(KSEB_SECTIONS)) {
                    for (const secName of Object.keys(sections)) {
                        const secUpper = secName.toUpperCase();
                        const reg = new RegExp('\\b' + secUpper + '\\b', 'i');
                        if (reg.test(lineUpper)) {
                            sectionName = secName;
                            districtName = dist;
                            console.log("Matched section via explicit line search:", sectionName, "(", districtName, ") Line:", line);
                            break;
                        }
                    }
        ...
        // Strategy B: If no explicit header line matched, find section with EARLIEST position in text (header wins over footer!)
        if (!sectionName) {
            ...
            for (const [dist, sections] of Object.entries(KSEB_SECTIONS)) {
                for (const secName of Object.keys(sections)) {
                    const secUpper = secName.toUpperCase();
                    const reg = new RegExp('\\b' + secUpper + '\\b', 'i');
                    const match = reg.exec(textUpper);
                    if (match) {
            ...
            if (sectionName) {
                console.log("Matched section via earliest position index:", sectionName, "(", districtName, ") Pos:", earliestPos);
            }
        }
    }

    if (!sectionName) {
        const secMatch = text.match(/(?:Electrical\s*Section|Section\s*Office|KSEB\s*Section|Section\s*Name|Section)[\s\:\-\=]*([A-Za-z\s]{3,25})/i);
        if (secMatch && secMatch[1]) {
            const rawSec = secMatch[1].trim();
            if (!/Office|Bill|Consumer|Details|Net|Total|Amount/i.test(rawSec)) {
                sectionName = rawSec;
            }
        }
    }
    ...
    // =========================================================================
    // 6. BILLING PERIOD CHECK
    // =========================================================================
    if (/Monthly|1\s*Month|30\s*Days/i.test(text) && !/Bi-Monthly|Bi\s*Monthly|60\s*Days|2\s*Month/i.test(text)) {
        isBiMonthly = false;
    }
...
// --- AI KSEB Bill Instant Scanner Engine ---
function handleKSEBBillUpload(event) {
    const file = event.target.files[0];
    if (!file) return;

    const dropzone = document.getElementById('bill-dropzone');
    const progressBox = document.getElementById('bill-scan-progress');
    const progressBar = document.getElementById('scan-progress-bar');
    const statusText = document.getElementById('scan-status-text');
    const percentText = document.getElementById('scan-percent-text');
    const successBox = document.getElementById('bill-scan-success');
    const filenameLabel = document.getElementById('scanned-filename');
    ...
    const scanStages = [
        { pct: 25, msg: "🔍 Scanning KSEB Document Header & Barcode..." },
        { pct: 55, msg: "⚡ Extracting 13-Digit Consumer No & Section..." },
        { pct: 85, msg: "📊 Calculating Bi-Monthly Units & Connected Load..." },
        { pct: 100, msg: "✅ Sizing Optimal Rooftop Solar Capacity..." }
    ];

    let stageIdx = 0;
    const interval = setInterval(() => {
        if (stageIdx < scanStages.length) {
            const current = scanStages[stageIdx];
            progressBar.style.width = current.pct + '%';
            percentText.textContent = current.pct + '%';
            statusText.textContent = current.msg;
            stageIdx++;
        } else {
            ...
            // Extract simulated realistic KSEB parameters
            const extractedConsumerNo = "1155442200" + Math.floor(100 + Math.random() * 900);
            const extractedUnits = 360;
            const recommendedKw = "3.0";

            if (document.getElementById('res-consumer-no')) {
                document.getElementById('res-consumer-no').textContent = extractedConsumerNo;
            }

            // Sync to Contact/Feasibility Form
            const formConsumerNo = document.getElementById('form-consumer-no');
            if (formConsumerNo) formConsumerNo.value = extractedConsumerNo;

            const formSize = document.getElementById('form-size');
            if (formSize) formSize.value = recommendedKw;

            const formMsg = document.getElementById('form-message');
            if (formMsg) {
                formMsg.value = `[Auto-Extracted from KSEB Bill]: Consumer No: ${extractedConsumerNo}, Monthly Consumption: ${extractedUnits} Units, Recommended Solar Capacity: ${recommendedKw} kWp On-Grid.`;
            }

            // Sync to Solar Calculator
            const calcUnits = document.getElementById('input-units');
            if (calcUnits) {
                calcUnits.value = extractedUnits;
                if (typeof handleUnitInput === 'function') handleUnitInput(extractedUnits);
            }
        }
    }, 400);
}
